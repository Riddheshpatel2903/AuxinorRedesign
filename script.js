    ...
    // Company sign on left wall
    place(B(0.04, 1.4, 3.5, M.sign), -9.97, 4.5, -5, 0, Math.PI / 2);
    place(B(0.04, 1.2, 3.0, M.ceil), -9.95, 4.5, -5, 0, Math.PI / 2); // sign face
    ...
    function showScene(sc) {
      ...
      // Side panels
      ['ip1', 'ip3'].forEach(id => { document.getElementById(id).classList.remove('on') });
      if (PANELS[sc]) setTimeout(() => document.getElementById(PANELS[sc]).classList.add('on'), 200);
      // Overlays
      ['pgrid', 'icloud', 'cbox'].forEach(id => { document.getElementById(id).classList.remove('on') });
      if (OVERLAYS[sc]) setTimeout(() => document.getElementById(OVERLAYS[sc]).classList.add('on'), 280);
      // Stats
