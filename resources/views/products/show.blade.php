...
<div class="bg-bg border-b border-line py-4 px-4 md:px-8">
 <div class="max-w-[1400px] mx-auto font-sans font-bold text-[10px] uppercase tracking-[2px] text-muted">
...
<section class="py-16 px-4 md:px-8 bg-white" x-data="{ activeTab: 'overview' }">
 ...
 <div class="flex flex-wrap items-center gap-3 mb-6">
                    @if($product->cas_number)
                        <span class="border border-teal/30 bg-teal-light text-teal font-sans font-bold text-[10px] uppercase tracking-widest px-3 py-1">CAS: {{ $product->cas_number }}</span>
                    @endif
                    @if($product->chemical_formula)
                        <span class="bg-bg border border-line text-ink font-sans font-bold text-[10px] uppercase tracking-widest px-3 py-1">Formula: {{ $product->chemical_formula }}</span>
                    @endif
                </div>
                
                <h1 class="font-display font-extrabold text-[40px] md:text-[64px] leading-tight text-ink mb-4">{{ $product->name }}</h1>
                <p class="font-sans italic text-[16px] text-muted leading-relaxed max-w-2xl">{{ $product->short_description ?? 'High-purity industrial grade compound suitable for bulk manufacturing and specialized applications.' }}</p>
            </div>
            ...
            <div class="mb-12">
                <div class="mb-4 overflow-hidden bg-bg border border-line aspect-[21/9]">
                    @php
                        $fallbackMain = asset('assets/images/product-1.jpg');
                        $mainImg = $product->image ? (str_starts_with($product->image, 'http') ? $product->image : Storage::url($product->image)) : $fallbackMain;
                    @endphp
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div class="overflow-hidden bg-bg border border-line aspect-square group">
                        <img src="{{ str_starts_with($galImg, 'http') ? $galImg : Storage::url($galImg) }}" alt="{{ $product->name }} gallery" class="w-full h-full object-cover transform transition-transform duration-700 group-hover:scale-110">
                    </div>
                    @endforeach
                </div>
                @endif
            ...
            <div class="min-h-[300px]">
                <!-- Overview -->
 <div x-show="activeTab === 'overview'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="prose prose-sm max-w-none font-sans text-muted leading-relaxed">
 ...
 <ul class="space-y-4">
                        @forelse($product->applications_list as $app)
                            <li class="flex items-start space-x-3 bg-bg p-4 border border-line">
                                ...
                                <span class="font-sans text-[14px] text-ink">{{ $app }}</span>
                            </li>
                        @empty
                            <div class="bg-bg p-6 text-center border text-muted font-sans text-[14px]">Specific applications are not listed. Often used in {{ $product->category->name }} derivatives.</div>
                        @endforelse
                    </ul>
                </div>
 ...
 <div x-show="activeTab === 'specifications'" style="display: none;" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
                            ...
                            <table class="w-full text-left border-collapse border border-line">
                                <tbody>
                                    ...
                                    <tr class="border-b border-line hover:bg-bg transition-colors">
                                        <th class="p-4 font-sans font-bold text-[10px] uppercase tracking-widest text-muted bg-white/50 w-1/3">{{ $spec['key'] ?? '' }}</th>
                                        <td class="p-4 font-sans text-[14px] text-ink border-l border-line">{{ $spec['value'] ?? '' }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    @else
                        <div class="bg-bg p-6 text-center border text-muted font-sans text-[14px]">Technical specifications available upon request.</div>
                    @endif
                </div>
            </div>
        </div>
 ...
 <div class="p-8 relative z-10">
                    ...
                    <p class="font-sans font-bold text-[10px] text-white/50 uppercase tracking-[2px] mb-8 border-b border-white/10 pb-4">Product: {{ $product->name }}</p>
                    ...
                    <form action="{{ route('contact.submit') }}" method="POST">
                        ...
                        <div class="mb-5">
                            <label class="block font-sans font-bold text-[9px] uppercase tracking-widest text-teal-2 mb-2">Company Name</label>
                            <input type="text" name="company_name" class="w-full bg-transparent border-b border-white/20 text-white font-sans focus:border-teal outline-none transition-colors pb-1">
                        </div>
                        <div class="mb-5">
                            <label class="block font-sans font-bold text-[9px] uppercase tracking-widest text-teal-2 mb-2">Contact Person *</label>
                            <input type="text" name="contact_person" required class="w-full bg-transparent border-b border-white/20 text-white font-sans focus:border-teal outline-none transition-colors pb-1">
                        </div>
                        <div class="mb-5">
                            <label class="block font-sans font-bold text-[9px] uppercase tracking-widest text-teal-2 mb-2">Email *</label>
                            <input type="email" name="email" required class="w-full bg-transparent border-b border-white/20 text-white font-sans focus:border-teal outline-none transition-colors pb-1">
                        </div>
                        <div class="mb-8">
                            <label class="block font-sans font-bold text-[9px] uppercase tracking-widest text-teal-2 mb-2">Phone</label>
                            <input type="tel" name="phone" class="w-full bg-transparent border-b border-white/20 text-white font-sans focus:border-teal outline-none transition-colors pb-1">
                        </div>
...
<section class="py-16 px-4 md:px-8 bg-bg border-t border-line">
 <div class="max-w-[1400px] mx-auto">
        <h3 class="font-display font-extrabold text-[28px] text-ink mb-8 pb-4 border-b border-line flex justify-between items-end">
            Related {{ $product->category->name }}
            <a href="{{ route('products.category', $product->category->slug) }}" class="font-sans font-bold text-[10px] uppercase tracking-[2px] text-teal hover:text-ink transition-colors pb-1">View All →</a>
        </h3>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            ...
            <a href="{{ route('products.show', ['category' => $rel->category->slug, 'slug' => $rel->slug]) }}" class="bg-white border border-line hover:border-teal group block transform transition-all hover:-translate-y-1">
                <div class="p-6">
                    <span class="font-sans font-bold text-[10px] uppercase tracking-widest text-muted block mb-2">{{ $rel->cas_number ? 'CAS ' . $rel->cas_number : 'NO CAS' }}</span>
