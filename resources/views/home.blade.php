...
@section('content')
    <!-- HERO SECTION -->
    <section data-section-id="1" data-section-key="hero" data-section-label="Hero Section"
        class="min-h-[88vh] border-b border-line relative overflow-hidden @auth cms-editable @endauth"
        data-cms-label="Edit Hero Layout & Images" x-data="{ 
                images: [
    ...
    <!-- SERVICES STRIP -->
    <section data-section-id="2" data-section-key="services_strip" data-section-label="Services Strip"
        class="bg-ink grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 p-4 lg:p-8 gap-4 sr-up relative z-10">
        <!-- Card 1: Trading -->
        <a href="#"
            ...
            <img src="{{ asset('assets/images/service-trading.jpg') }}"
                class="absolute inset-0 w-full h-full object-cover filter brightness-75 group-hover:scale-105 group-hover:brightness-100 transition-all duration-700"
                alt="Chemical Trading Data">
            <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-90"></div>
            <div class="absolute inset-0 p-8 flex flex-col justify-between">
                <h3
                    class="font-display font-medium text-3xl text-white leading-tight max-w-[200px] group-hover:text-4xl group-hover:tracking-tight transition-all duration-500">
                    Chemical<br>Trading</h3>
                <div
                    class="inline-flex items-center justify-center space-x-2 border border-white/40 rounded-full px-5 py-2.5 w-max backdrop-blur-md hover:bg-white hover:text-ink transition-all">
                    <span
        ...
        <a href="#"
            class="relative h-[350px] lg:h-[450px] bg-ink2 overflow-hidden group rounded-sm block hover:-translate-y-2 transition-transform duration-500">
            <img src="{{ asset('assets/images/service-logistics.jpg') }}"
                class="absolute inset-0 w-full h-full object-cover filter brightness-75 group-hover:scale-105 group-hover:brightness-100 transition-all duration-700"
        ...
        <a href="#"
            class="relative h-[350px] lg:h-[450px] bg-ink2 overflow-hidden group rounded-sm block hover:-translate-y-2 transition-transform duration-500">
            <img src="{{ asset('assets/images/service-warehousing.jpg') }}"
                class="absolute inset-0 w-full h-full object-cover filter brightness-75 group-hover:scale-105 group-hover:brightness-100 transition-all duration-700"
                alt="Bulk Warehousing">
            <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-90"></div>
            <div class="absolute inset-0 p-8 flex flex-col justify-between">
                <h3
                    class="font-display font-medium text-3xl text-white leading-tight max-w-[200px] group-hover:text-4xl group-hover:tracking-tight transition-all duration-500">
        ...
        <a href="#"
            class="relative h-[350px] lg:h-[450px] bg-ink2 overflow-hidden group rounded-sm block hover:-translate-y-2 transition-transform duration-500">
            <img src="{{ asset('assets/images/service-insights.jpg') }}"
                class="absolute inset-0 w-full h-full object-cover filter brightness-75 group-hover:scale-105 group-hover:brightness-100 transition-all duration-700"
                alt="Market Insights Trading">
            <div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-90"></div>
            <div class="absolute inset-0 p-8 flex flex-col justify-between">
                <h3
                    class="font-display font-medium text-3xl text-white leading-tight max-w-[200px] group-hover:text-4xl group-hover:tracking-tight transition-all duration-500">
    ...
    <!-- INDUSTRIES SECTION -->
    <section data-section-id="3" data-section-key="industries" data-section-label="Industries"
        class="bg-white py-[100px] px-4 md:px-8 max-w-[1400px] mx-auto">
        <div class="mb-16">
            <span class="font-mono text-[10px] uppercase tracking-widest text-teal block mb-4">Industries Served</span>
            <h2 class="font-display font-extrabold text-[48px] leading-tight">Where We <em
                    class="font-serif italic text-teal font-normal">Deliver</em></h2>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sr-stagger">
            ...
            @foreach($industries as $idx => $industry)
                <a href="{{ route('industries') }}"
                    ...
                    <!-- Background Image -->
                    <img src="{{ $industryImages[$idx % count($industryImages)] }}"
                    ...
                    <div
                        class="absolute inset-0 bg-teal/40 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-0 mix-blend-multiply">
                    </div>
    ...
    <!-- PRODUCTS SECTION -->
    <section data-section-id="4" data-section-key="products" data-section-label="Products"
        class="bg-bg py-[100px] px-4 md:px-8 border-y border-line">
        <div class="max-w-[1400px] mx-auto">
            <div class="flex flex-col md:flex-row justify-between items-end mb-16">
                <div>
                    <span class="font-mono text-[10px] uppercase tracking-widest text-teal block mb-4">Catalogue</span>
                    <h2 class="font-display font-extrabold text-[48px] leading-tight">Our <em
                            class="font-serif italic text-teal font-normal">Range</em></h2>
                </div>
                <a href="{{ route('products.index') }}"
            ...
            <div class="flex flex-wrap gap-2 mb-12 border border-line p-2 bg-white pf-container">
                ...
                @foreach($categories->take(4) as $cat)
                    <button class="pf px-4 py-2 font-mono text-[9px] uppercase tracking-wider bg-white text-ink hover:bg-bg"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 product-grid">
                ...
                @foreach($featuredProducts as $idx => $product)
                    <a href="{{ route('products.show', ['category' => $product->category->slug ?? 'misc', 'slug' => $product->slug]) }}"
                        ...
                        style="--ds: {{ $idx * 85 }}ms">
                        <!-- Background Image -->
                        <img src="{{ $product->image_url ?? $containerImages[$idx % count($containerImages)] }}"
    ...
    <section data-section-id="5" data-section-key="about_infra" data-section-label="About & Infrastructure"
        ...
        @if(($globalSettings['show_infra_image'] ?? '1') == '1' && !empty($globalSettings['infra_image_url']))
            <img src="{{ $globalSettings['infra_image_url'] }}"
        ...
        <div class="max-w-[1400px] relative z-10 mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20">
            ...
            <div class="flex flex-col justify-center">
                ...
                <div class="space-y-6 mb-12 sr-l data-delay-200">
                    <p data-element-id="el_setting:about_short"
                        class="font-serif italic text-[14px] text-[#5a7080] leading-relaxed">
                    ...
                    <p data-element-id="el_setting:about_long"
                        class="font-serif italic text-[14px] text-[#5a7080] leading-relaxed">
                ...
                <div class="grid grid-cols-3 bg-ink2 border border-white/10 p-6 sr-l data-delay-300">
                    <div class="text-center border-r border-white/10">
                        <div class="font-display font-bold text-2xl text-white mb-1" data-count="{{ $stats['years'] }}">0
                        </div>
                        <div class="font-mono text-[9px] uppercase tracking-wider text-white/50">Years</div>
                    </div>
                    <div class="text-center border-r border-white/10">
                        <div class="font-display font-bold text-2xl text-white mb-1" data-count="{{ $stats['products'] }}">0
                        </div>
                        <div class="font-mono text-[9px] uppercase tracking-wider text-white/50">Products</div>
                    </div>
                    <div class="text-center">
                        <div class="font-display font-bold text-2xl text-white mb-1"
                            data-count="{{ $stats['industries'] }}">0</div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="bg-[#111b25] border border-[#1a2535] p-8 hover:border-teal hover:bg-[#0d1e1a] transition-colors sr-r group"
                    data-delay="0">
                    <div
                        class="w-12 h-12 bg-ink border border-[#1e2d3d] flex items-center justify-center text-2xl mb-6 text-teal-2 transition-transform duration-500 group-hover:scale-110 group-hover:rotate-[-5deg]">
                        &#x1F3ED;</div>
                    <h3 class="font-display font-bold text-white text-[16px] mb-3 tracking-wide">Warehousing Facilities</h3>
                    <p class="font-serif italic text-[13px] text-[#6a8090] leading-relaxed">Safe storage for bulk organics
                        and inorganics.</p>
                </div>
                <div class="bg-[#111b25] border border-[#1a2535] p-8 hover:border-teal hover:bg-[#0d1e1a] transition-colors sr-r group"
                    data-delay="130">
                    ...
                    <p class="font-serif italic text-[13px] text-[#6a8090] leading-relaxed">Timely dispatch and seamless
                        supply chain.</p>
                </div>
                <div class="bg-[#111b25] border border-[#1a2535] p-8 hover:border-teal hover:bg-[#0d1e1a] transition-colors sr-r group"
                    data-delay="260">
                    ...
                    <p class="font-serif italic text-[13px] text-[#6a8090] leading-relaxed">Strong alliances with global
                        manufacturers.</p>
                </div>
                <div class="bg-[#111b25] border border-[#1a2535] p-8 hover:border-teal hover:bg-[#0d1e1a] transition-colors sr-r group"
                    data-delay="390">
                    ...
                    <p class="font-serif italic text-[13px] text-[#6a8090] leading-relaxed">Always-in-stock policy for key
    ...
    <!-- MARKET INSIGHTS -->
    <section data-section-id="6" data-section-key="insights" data-section-label="Market Insights"
        class="bg-white py-[100px] px-4 md:px-8">
        <div class="max-w-[1400px] mx-auto">
            <div class="flex flex-col md:flex-row justify-between items-end mb-16">
                <div>
                    <span class="font-mono text-[10px] uppercase tracking-widest text-teal block mb-4">Intelligence</span>
                    <h2 class="font-display font-extrabold text-[48px] leading-tight text-ink">Market <em
                            class="font-serif italic text-teal font-normal">Insights</em></h2>
                </div>
    ...
    <!-- CONTACT SECTION -->
    <section data-section-id="7" data-section-key="contact" data-section-label="Contact & Enquiry"
        class="bg-bg py-[100px] px-4 md:px-8 border-t border-line @auth cms-editable @endauth"
        data-cms-label="Edit Contact Details">
