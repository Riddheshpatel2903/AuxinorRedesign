<footer data-section-id="9999" data-section-key="footer" data-section-label="Global Footer" class="bg-ink text-white pt-20 pb-6 border-t-[4px] border-teal">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
            ...
            <div>
                <a href="{{ route('home') }}" class="flex items-center mb-6">
                    <div class="h-[50px] bg-white p-1 rounded-sm shadow-sm border border-white/10">
                        @php
 ...
 <p data-element-id="setting:footer_tagline" class="font-sans italic text-[13px] text-white/60 mb-6 leading-relaxed">
            ...
            <div>
                ...
                <ul class="space-y-3">
                    @php
                        $footerCats = is_object($globalCategories) ? $globalCategories->take(5) : array_slice($globalCategories ?? [], 0, 5);
                    @endphp
 @foreach($footerCats as $cat)
                        <li><a href="{{ route('products.category', $cat->slug) }}" class="font-sans text-[12px] text-white/60 hover:text-teal-2 transition-colors">{{ $cat->name }}</a></li>
                    @endforeach
                    <li><a href="{{ route('products.index') }}" class="font-sans font-bold text-[11px] text-teal hover:text-white transition-colors mt-2 inline-block uppercase tracking-wider">View All Products →</a></li>
                </ul>
            </div>
 ...
 <ul class="space-y-3 font-sans text-[12px]">
            ...
            <div>
                ...
                <div class="space-y-4">
 <div>
                        <span class="block font-sans font-bold text-[9px] uppercase tracking-widest text-teal-2 mb-1">Corporate Office</span>
                        <p class="font-sans text-[13px] text-white/60">{{ $globalSettings['office_address'] ?? 'Ahmedabad, Gujarat' }}</p>
                    </div>
                    <div>
                        <span class="block font-sans font-bold text-[9px] uppercase tracking-widest text-teal-2 mb-1">Call Us</span>
                        <a href="tel:{{ $globalSettings['phone_primary'] ?? '' }}" class="font-sans font-medium text-[14px] text-white hover:text-teal-2 transition-colors block">{{ $globalSettings['phone_primary'] ?? '+91' }}</a>
                    </div>
                    <div>
