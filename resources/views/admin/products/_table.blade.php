        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b border-gray-200">
                    <th class="px-6 py-4 font-mono text-[10px] uppercase tracking-wider text-gray-500 w-16">Status</th>
                    <th class="px-6 py-4 font-mono text-[10px] uppercase tracking-wider text-gray-500">Product Name / CAS</th>
                    <th class="px-6 py-4 font-mono text-[10px] uppercase tracking-wider text-gray-500">Category</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors group">
                    ...
                    <td class="px-6 py-4">
                        <div class="font-display font-bold text-[15px] text-ink group-hover:text-teal transition-colors">{{ $product->name }}</div>
                        <div class="font-mono text-[10px] text-gray-400 mt-1 uppercase">
                            CAS: {{ $product->cas_number ?: 'N/A' }} 
                            @if($product->chemical_formula) | Formula: {{ $product->chemical_formula }} @endif
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <span class="inline-block px-2 py-1 bg-gray-100 text-gray-600 font-mono text-[10px] rounded uppercase tracking-wider">{{ $product->category->name ?? 'Uncategorized' }}</span>
                    </td>
                    <td class="px-6 py-4 text-right space-x-2">
                        <a href="{{ route('admin.products.edit', $product) }}" class="text-gray-400 hover:text-ink transition-colors p-1" title="Edit">
                        ...
                        <a href="{{ route('products.show', [$product->category->slug ?? 'misc', $product->slug]) }}" target="_blank" class="text-gray-400 hover:text-teal transition-colors p-1" title="View on Frontend">
