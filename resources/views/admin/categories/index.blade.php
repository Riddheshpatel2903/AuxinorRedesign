        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="border-b border-gray-200 bg-gray-50">
                    <th class="px-6 py-4 font-mono text-[10px] uppercase tracking-wider text-gray-500 w-16">Icon</th>
                    <th class="px-6 py-4 font-mono text-[10px] uppercase tracking-wider text-gray-500">Details</th>
                    <th class="px-6 py-4 font-mono text-[10px] uppercase tracking-wider text-gray-500 text-center w-24">Products</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition-colors group">
                    <td class="px-6 py-4">
                        <div class="w-10 h-10 border border-gray-200 bg-white shadow-sm flex items-center justify-center text-xl text-teal">
                            {{ $category->icon }}
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="font-display font-bold text-[15px] text-ink group-hover:text-teal transition-colors">{{ $category->name }}</div>
                        <div class="font-mono text-[10px] text-gray-400 mt-1">/{{ $category->slug }}</div>
                        @if($category->description)
                            <div class="font-serif italic text-[12px] text-gray-600 mt-1 line-clamp-1">{{ $category->description }}</div>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-center">
                        <span class="inline-block px-2 py-1 bg-gray-100 text-gray-700 font-mono text-[10px] rounded">{{ $category->products_count ?? $category->products()->count() }}</span>
                    </td>
                    <td class="px-6 py-4 text-right space-x-2">
                        <a href="{{ route('admin.categories.edit', $category) }}" class="text-gray-400 hover:text-ink transition-colors p-1" title="Edit">
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-6 py-12 text-center text-gray-400 font-serif italic text-sm">
