...
@section('subheader', 'Update industry details, description or visual assets.')
    ...
    <div class="admin-card p-8">
        <form action="{{ route('admin.industries.update', $industry) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" required value="{{ old('name', $industry->name) }}" class="admin-input p-3 w-full font-display font-bold text-lg">
                    ...
                    <div>
                        ...
                        <input type="text" name="slug" value="{{ old('slug', $industry->slug) }}" class="admin-input p-3 w-full font-mono text-xs">
                    ...
                    <div class="flex gap-6">
                        <div class="flex-1">
                            ...
                            <input type="text" name="icon" value="{{ old('icon', $industry->icon) }}" class="admin-input p-3 w-full text-center text-2xl">
                        </div>
                        <div class="w-32">
                            <label class="admin-label">Sort Order</label>
                            <input type="number" name="sort_order" value="{{ old('sort_order', $industry->sort_order) }}" class="admin-input p-3 w-full text-center font-mono">
                        </div>
                    </div>

                    <div class="flex items-center p-4 bg-gray-50 border border-line">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $industry->is_active) ? 'checked' : '' }} class="w-4 h-4 text-teal focus:ring-teal border-gray-300">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <textarea name="desc" rows="6" class="admin-input p-4 w-full font-serif text-sm leading-relaxed">{{ old('desc', $industry->desc) }}</textarea>
                    ...
                    <div>
                        ...
                        <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-100 border-dashed hover:border-teal/30 transition-colors cursor-pointer relative group">
                            ...
                            <div class="space-y-1 text-center">
                                ...
                                <div class="text-xs text-muted font-mono tracking-tight">
                                    <span class="text-teal font-bold uppercase">Change File</span>
                                    <p class="mt-1">PNG, JPG, WEBP up to 5MB</p>
                                </div>
                            </div>

                            @if($industry->image)
                                <img id="preview" src="{{ Storage::url($industry->image) }}" class="absolute inset-0 w-full h-full object-cover z-0 opacity-60 group-hover:opacity-40 transition-opacity">
                            @else
            ...
            <div class="flex justify-end gap-3 border-t border-line pt-8">
                ...
                <button type="submit" class="bg-ink text-white px-8 py-3 font-display font-bold text-[11px] uppercase tracking-widest hover:bg-teal transition-all shadow-lg">Update Industry</button>
            </div>
        </form>
    </div>
</div>
